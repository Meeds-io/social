...
public class SpaceAccessHandler extends WebRequestHandler {
  ...
  public static final String PARENT_SPACE_REGISTRATION_TYPE = "parentRegistrationType";
  ...
  public boolean execute(ControllerContext controllerContext) throws Exception {
    ...
    if (canAccessSpace(space, username)) {
      ...
      String parentSpaceRegistration = null;
      if (space.getParentSpaceId() != null) {
        Space parentSpace = spaceService.getSpaceById(space.getParentSpaceId());
        parentSpaceRegistration = parentSpace.getRegistration();
      }
      processSpaceAccess(controllerContext, username, space, invitationId, parentSpaceRegistration);
      return true;
    }
  }
  ...
  private void processSpaceAccess(ControllerContext controllerContext,
                                  ...
                                  Space space,
                                  String invitationId,
                                  String parentSpaceRegistration) throws IOException {
    SpaceAccessType spaceAccessType = Arrays.stream(SpaceAccessType.values())
                                            .filter(accessType -> accessType.doCheck(remoteId, space))
                                            .findFirst()
                                            .orElse(null);
    sendRedirect(controllerContext, spaceAccessType, space, invitationId, parentSpaceRegistration);
  }
  ...
  private void sendRedirect(ControllerContext controllerContext,
    ...
    if (parentSpaceRegistration != null) {
      redirectUrl.append(hasQuery ? "&" : "?")
                 .append(PARENT_SPACE_REGISTRATION_TYPE)
                 .append("=")
                 .append(URLEncoder.encode(parentSpaceRegistration, StandardCharsets.UTF_8));
    }
