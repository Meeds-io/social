...
public class EntityBuilder {
    ...
    if (isAdmin) {
      ...
      try {
        ...
        if (user != null) {
          ...
          // the super user account can never be deleted nor disabled: the
          // administration UI must not offer those actions for it
          userEntity.setIsSuperUser(StringUtils.equals(getUserACL().getSuperUser(), user.getUserName()));
  ...
  private static boolean canReportActivity(ExoSocialActivity activity, Identity authentiatedUser) {
    ...
    // read-path eligibility only: resolves through the local, user-first
    // helper shared with the other stream properties of this payload, while
    // the create guard resolves through
    // ActivityManager.getActivityStreamOwnerIdentity. The only divergence (a
    // username equal to a space pretty name) hides the entry while the write
    // guard would accept the report — safe direction, the guard stays the
    // authority
  ...
  private static void populateGeneralFields(SpaceEntity spaceEntity, Space space) {
  ...
  private static void setPublicSiteDetails(SpaceEntity spaceEntity, Space space) {
    if (space.getPublicSiteId() > 0) {
      PortalConfig publicPortalConfig = getLayoutService().getPortalConfig(space.getPublicSiteId());
      if (publicPortalConfig == null || !getUserACL().hasAccessPermission(publicPortalConfig, getCurrentUserIdentity())) {
