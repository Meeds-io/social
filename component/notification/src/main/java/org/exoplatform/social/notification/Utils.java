...
public class Utils {
   ...
   */
  public static void sendToActivityPoster(Set<String> receivers, String activityPosterId, String posteId, ActivityStream activityStream) {
    String activityPosterRemoteId = Utils.getUserId(activityPosterId);
    if (activityPosterId.equals(posteId) == false && canAddPosterToReceivers(activityStream, activityPosterRemoteId)) {
      receivers.add(activityPosterRemoteId);
    }
  }

  public static boolean canAddPosterToReceivers(ActivityStream activityStream, String activityPosterRemoteId) {
    if (activityStream.getType().getProviderId().equals("space")){
      ...
      if(!spaceService.isMember(space, activityPosterRemoteId)) {
        return false;
      }
      return true;
    }
