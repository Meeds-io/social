 ...
 + authorizeUrl);
  ...
  public boolean processLogout(HttpServletRequest request, HttpServletResponse response, OAuthProviderType oAuthProviderType) throws IOException {
    ...
    if (logoutStateFromSession != null) {
      ...
      // Detection only: a mismatch is logged for observability but never
      // blocks — gating here would leave a genuine self-initiated logout
      // (e.g. concurrent tabs, which legitimately overwrite the stored
      // state) incomplete on this side. The local session ends either way.
