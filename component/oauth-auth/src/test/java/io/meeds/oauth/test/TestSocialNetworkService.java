...
public class TestSocialNetworkService extends AbstractKernelTest {
  ...
  public void setUp() throws Exception {
    super.setUp();
    this.oAuthProviderTypeRegistry = getContainer().getComponentInstanceOfType(OAuthProviderTypeRegistry.class);
    this.orgService = getContainer().getComponentInstanceOfType(OrganizationService.class);
    this.socialNetworkService = getContainer().getComponentInstanceOfType(SocialNetworkService.class);
    this.codec = getContainer().getComponentInstanceOfType(CodecInitializer.class).getCodec();
    begin();
  }
  ...
  public void testPersistOAuthProviderUsernames() throws Exception {
    ...
    socialNetworkService.updateOAuthInfo(getFacebookProvider(),
                                         user1.getUserName(),
                                         "joseph.doyle.changed",
                                         createFacebookAccessToken("someToken"));
    ...
    try {
      ...
      assertEquals(OAuthConstants.PROFILE_FACEBOOK_USERNAME,
                   gtnOauthOAuthException.getExceptionAttribute(OAuthConstants.EXCEPTION_OAUTH_PROVIDER_USERNAME_ATTRIBUTE_NAME));
      assertEquals("joseph.doyle.changed",
                   gtnOauthOAuthException.getExceptionAttribute(OAuthConstants.EXCEPTION_OAUTH_PROVIDER_USERNAME));
    } catch (Exception e) {
  ...
  public void testPersistOAuthAccessTokens() throws Exception {
    ...
    OAuthProviderType<FacebookAccessTokenContext> facebookProvider = getFacebookProvider();
    socialNetworkService.updateOAuthAccessToken(facebookProvider, user1.getUserName(), createFacebookAccessToken("aaa123"));
    socialNetworkService.updateOAuthAccessToken(facebookProvider, user2.getUserName(), createFacebookAccessToken("bbb456"));
    ...
    assertEquals("aaa123", socialNetworkService.getOAuthAccessToken(facebookProvider, user1.getUserName()).getAccessToken());
    assertEquals("bbb456", socialNetworkService.getOAuthAccessToken(facebookProvider, user2.getUserName()).getAccessToken());
    ...
    assertEquals(new TwitterAccessTokenContext("tok1", "secret1"),
                 socialNetworkService.getOAuthAccessToken(getTwitterProvider(), user1.getUserName()));
    assertNull(socialNetworkService.getOAuthAccessToken(getTwitterProvider(), user2.getUserName()));

    // Directly obtain accessTokens from userProfile and verify that they are
    // encoded
    ...
    socialNetworkService.removeOAuthAccessToken(facebookProvider, user1.getUserName());
    ...
    assertNull(socialNetworkService.getOAuthAccessToken(facebookProvider, user1.getUserName()));
    ...
    assertNotNull(socialNetworkService.getOAuthAccessToken(facebookProvider, user2.getUserName()));
  ...
  public void testInvalidationOfAccessTokens() throws Exception {
    ...
    socialNetworkService.updateOAuthInfo(getFacebookProvider(),
                                         user1.getUserName(),
                                         "fbUsername1",
                                         createFacebookAccessToken("fbAccessToken1"));
    assertEquals("fbAccessToken1",
                 socialNetworkService.getOAuthAccessToken(getFacebookProvider(), user1.getUserName()).getAccessToken());

    // Update some accessToken again
    socialNetworkService.updateOAuthInfo(getFacebookProvider(),
                                         user1.getUserName(),
                                         "fbUsername1",
                                         createFacebookAccessToken("fbAccessToken2"));
    assertEquals("fbAccessToken2",
                 socialNetworkService.getOAuthAccessToken(getFacebookProvider(), user1.getUserName()).getAccessToken());

    // Update userProfile and change FB username. AccessToken should be
    // invalidated
    ...
    socialNetworkService.updateOAuthAccessToken(getFacebookProvider(),
                                                user1.getUserName(),
                                                createFacebookAccessToken("fbAccessToken3"));
    assertEquals("fbAccessToken3",
                 socialNetworkService.getOAuthAccessToken(getFacebookProvider(), user1.getUserName()).getAccessToken());
  ...
  public void testLongAccessToken() throws Exception {
    ...
    socialNetworkService.updateOAuthInfo(getFacebookProvider(),
                                         user1.getUserName(),
                                         "fbUsername1",
                                         createFacebookAccessToken(longAccessToken));
    assertEquals(longAccessToken,
                 socialNetworkService.getOAuthAccessToken(getFacebookProvider(), user1.getUserName()).getAccessToken());
    ...
    socialNetworkService.updateOAuthInfo(getFacebookProvider(),
                                         user1.getUserName(),
                                         "fbUsername1",
                                         createFacebookAccessToken(shortAccessToken));
    assertEquals(shortAccessToken,
                 socialNetworkService.getOAuthAccessToken(getFacebookProvider(), user1.getUserName()).getAccessToken());
  ...
  public void testAccessTokensWithMoreScopes() throws Exception {
    ...
    // Verify that Google accessTokens could be obtained and are equals to saved
    // access token
    assertEquals(googleToken, socialNetworkService.getOAuthAccessToken(getGoogleProvider(), user1.getUserName()));
    ...
    // Verify that Facebook accessTokens could be obtained and are equals to
    // saved access token
  ...
  private String createLongString() {
    StringBuilder builder =
                          new StringBuilder("1324567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890");
    for (int i = 0; i < 3; i++) {
