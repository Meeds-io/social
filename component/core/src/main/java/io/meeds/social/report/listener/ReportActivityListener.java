...
public class ReportActivityListener extends ActivityListenerPlugin {
  ...
  public void updateActivity(ActivityLifeCycleEvent event) {
    markReportsStaleOnContentChange(event);
  }
  ...
  private void markReportsStaleOnContentChange(ActivityLifeCycleEvent event) {
    // the Reported state holds until the content itself changes: the
    // update lifecycle is also broadcast for content-neutral updates
    // (category link/unlink, unhide, ...) which must not re-enable reporting
