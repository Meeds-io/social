...
public class ActivityDAOImpl extends GenericDAOJPAImpl<ActivityEntity, Long> implements ActivityDAO {
  ...
  private String getQueryFilterContent(ActivityFilter activityFilter, List<String> predicates, boolean count) {
    String querySelect = count ? "SELECT COUNT(item.activity.id)" : "SELECT DISTINCT(item.activity.id), item.activity.pinDate, item.activity.updatedDate";
    querySelect = querySelect + " FROM SocStreamItem item";

    String orderBy = activityFilter.isShowPinned() ? " ORDER BY item.activity.pinDate DESC NULLS LAST, item.activity.updatedDate DESC NULLS LAST" : " ORDER BY item.activity.updatedDate DESC NULLS LAST";
