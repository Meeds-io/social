...
public class ActivityReportServiceTest extends AbstractCoreTest {
  ...
  public void testUpdateActivityLifecycleCarriesContentChangeFlag() throws Exception {
    ExoSocialActivity activity = createRedirectedSpaceActivity("ContentFlagSpace");

    List<org.exoplatform.social.core.activity.ActivityLifeCycleEvent> events = new ArrayList<>();
    boolean[] capturing = { true };
    // there is no removeActivityEventListener: keep the capture guarded so the
    // listener becomes inert for the rest of the suite
    activityManager.addActivityEventListener(new org.exoplatform.social.core.activity.ActivityListenerPlugin() {
      @Override
    ...
    try {
      // content-neutral update: the exact shape CategoryLinkModifiedListener
      // and the unhide endpoint use — reload then update without touching
      // title/body. It must broadcast contentChanged=false so the report
      // stale flip (US03: reset on author edit only) is not triggered
      ExoSocialActivity stored = activityManager.getActivity(activity.getId());
      activityManager.updateActivity(stored);
      assertEquals(1, events.size());
      assertFalse("A content-neutral update must broadcast contentChanged=false",
                  events.get(0).isContentChanged());

      ExoSocialActivity edited = activityManager.getActivity(activity.getId());
      edited.setTitle("An edited article activity");
      activityManager.updateActivity(edited);
      assertEquals(2, events.size());
      assertTrue("A content edit must broadcast contentChanged=true", events.get(1).isContentChanged());
    } finally {
      capturing[0] = false;
    }
  }

  public void testReportOnRedirectedActivityEvictsCachedActivity() throws Exception {
    ExoSocialActivity activity = createRedirectedSpaceActivity("EvictedArticleSpace");
    org.exoplatform.services.cache.ExoCache<ActivityKey, ActivityData> activityCache = cacheService.getActivityCache();
    ActivityKey activityKey = new ActivityKey(activity.getId());

    activityManager.getActivity(activity.getId());
    assertNotNull("Test setup: the redirected activity must be cached before reporting",
                  activityCache.get(activityKey));

    // the redirected activity is cached under its redirected metadata object,
    // which the selector-based eviction never matches: without the explicit
    // by-key eviction, the cached activity keeps serving stale
    // canReport/hasReported flags after the report
    activityReportService.reportActivity(activity.getId(), "spam", marySecurityIdentity());
    assertNull("Creating a report on a redirected activity must evict it from the activity cache",
               activityCache.get(activityKey));

    ExoSocialActivity storedActivity = activityManager.getActivity(activity.getId());
    assertNotNull(activityCache.get(activityKey));
    activityReportService.markReportsStale(storedActivity);
    assertNull("Flipping reports stale on a redirected activity must evict it from the activity cache",
               activityCache.get(activityKey));
  }
