  ...
  <v-card
    ...
    <div id="spacesAdministrationListBody" class="flex-grow-1 flex-shrink-1 pt-2">
      <v-data-table
        ...
        <template v-if="$root.selectedSpaces.length" slot="body.prepend">
          <tr>
            <td :colspan="headers.length + 1" class="px-0">
              <v-alert
                ...
                <div
                  v-if="!$root.isBulkProcessing"
                  v-html="selectionLabel" />
                <div v-else>
                  {{ $t('social.spaces.administration.manageSpaces.processingSpacesBulkOperation', {
                    0: $root.selectedSpaces.length,
                  }) }}
                </div>
              </v-alert>
            </td>
