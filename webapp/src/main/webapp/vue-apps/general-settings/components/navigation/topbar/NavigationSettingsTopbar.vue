    ...
    <v-data-table
      ...
      <template slot="item.name" slot-scope="{item}">
        <div class="d-flex flex-column flex-sm-row align-center ms-n4 text-truncate">
          <v-card
            class="transparent me-0 me-sm-4"
            width="20"
            flat>
            <v-img
              v-if="item.imageUrl"
  ...
  methods: {
    ...
    refreshApplications() {
      if (this.applications?.length && this.applicationExtension) {
        this.applications
          .filter(app => app.type === 'EXTENSION' && !this.loadedApplications[app.id])
          .forEach(this.refreshApplication);
      }
    },
    async refreshApplication(app) {
      this.loadedApplications[app.id] = true;
      const topbarApplication = await this.applicationExtension.loadApplication(app);
      if (!topbarApplication?.id) {
