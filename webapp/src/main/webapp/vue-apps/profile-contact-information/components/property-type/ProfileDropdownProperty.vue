  ...
  <div :class="{'mx-4 mt-2': !multiValued}">
    ...
    <div class="d-flex align-center">
      <v-combobox
        ...
        outlined
        dense
        @update:search-input="filterUpdated" />
...
export default {
  ...
  methods: {
    ...
    getOptionsOwner() {
      return this.parentProperty || this.propertyObject;
    },
    clonePropertyObject() {
      this.propertyObject = this.parentProperty && this.property || structuredClone(this.property);
      this.selectedOption = this.getOptionsOwner().propertyOptions.find(
        option => `${option.id}` === `${this.propertyObject.value}`
      );
      if (this.selectedOption) {
        this.selectedOption.translatedValue ??= this.selectedOption?.value;
      }
    }
  }
};
