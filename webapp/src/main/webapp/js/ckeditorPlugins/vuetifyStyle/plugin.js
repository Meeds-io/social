...
*/
CKEDITOR.plugins.add( 'vuetifyStyle', {
  init: function( editor ) {
    editor.on('contentDom', addVuetifyClasses);

    function addVuetifyClasses() {
      addVuetifyClassesSync();
    }

    function addVuetifyClassesSync() {
      if (editor?.status && editor?.document?.getDocumentElement?.()) {
