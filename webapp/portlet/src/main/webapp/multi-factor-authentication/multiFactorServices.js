...
export function saveProtectedNavigations(navigations) {
  return fetch(`${eXo.env.portal.context}/${eXo.env.portal.rest}/mfa/saveProtectedNavigations `, {
    ...
    if (!resp || !resp.ok) {
      throw new Error('Error when saving MFA protected navigations', resp);
    } else {
      return resp.json();
    }
  });
...
export function getNavigations() {
  return fetch(`${eXo.env.portal.context}/${eXo.env.portal.rest}/v1/navigations/group?visibility=displayed`, {
    method: 'GET',
    credentials: 'include',
  }).then((resp) => {
    if (resp && resp.ok) {
