...
export default {
  ...
  computed: {
    displayBadge() {
      return this.spaceWebNotificationsEnabled && this.unread;
    },
  ...
  methods: {
    ...
    handleUpdatesFromWebSocket(event) {
      ...
      if (Number(this.spaceId) === Number(spaceId)
        ...
        if (wsEventName === 'notification.read.allItems') {
          ...
          if (applicationName === 'activity' && applicationId === this.applicationId) {
    ...
    markAsRead() {
      if (this.displayBadge) {
        this.$spaceService.markAsRead(this.spaceId, 'activity', this.applicationId);
      }
    },
  }
};
