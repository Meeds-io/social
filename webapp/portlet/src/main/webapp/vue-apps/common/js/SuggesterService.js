  ...
  return fetch(`/portal/rest/v1/social/spaces?${params}`, {credentials: 'include'})
    ...
    .then(data => {
      data.spaces.forEach((item) => {
        if ((excludeRedactionalSpace && !item.redactorsCount ) || (onlyRedactor && ((item.isRedactor && item.redactorsCount) || !item.redactorsCount )) || (onlyManager && item.canEdit) || (!onlyRedactor && !onlyManager && !excludeRedactionalSpace) ){
          items.push({
