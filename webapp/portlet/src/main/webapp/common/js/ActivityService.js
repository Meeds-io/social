...
export function likeActivity(id, expand) {
  return fetch(`${eXo.env.portal.context}/${eXo.env.portal.rest}/v1/social/activities/${id}/likes?expand=${expand || ''}`, {
    method: 'POST',
    credentials: 'include',
  }).then(resp => {
    if (!resp || !resp.ok) {
...
export function unlikeActivity(id, expand) {
  return fetch(`${eXo.env.portal.context}/${eXo.env.portal.rest}/v1/social/activities/${id}/likes?expand=${expand || ''}`, {
    method: 'DELETE',
    credentials: 'include',
  }).then(resp => {
